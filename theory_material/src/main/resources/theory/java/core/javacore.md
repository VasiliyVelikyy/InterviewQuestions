...
Lambda — это анонимная функция, которая может быть передана как аргумент в метод или сохранена в переменную.
Лямбда-выражения упрощают код, избавляя от необходимости создавать полноценные классы для реализации функциональных
интерфейсов. Они часто используются для краткого описания логики,
...
- Class -по умолчанию. В байт код попадет но при выполнении программы через reflection ее не увидим. Это нужно для АСТ
  трансформации, байт-код инструментария
...
Метод без возвращаемого типа совпадает с именем класса. Будет вызван перед созданием объекта.
Служит для инициализации переменных экземпляра.
Если не продекларирован никакой конструктор - то компилятор создаст конструктор по умолчанию (без парамеров).
Если имеется какой-то конструктор с параметрами - конструктор по умолчанию не создастся автоматически. Нужно будет его
явно декларировать в классе.
Как и с методами, допускается наличие нескольких конструкторов - главное условие чтобы они были перегружены - то-есть
отличались количеством и типом аргументов/
Переменным экземпляра будет присвоено значение по умолчанию, если они не проинициализированы.
...
При создании дочернего объекта, должен вызываться конструктор родительского по всей цепочке наследования (до класса
Object), даже если это
абстрактный класс (нельзя создать экземпляр абстрактного класса). Это нужно чтобы выполнить логику инициализации
переменных. Так как дочерний
объект имеет доступ ко свойствам родителя как к своим. Переменные могут быть закрыты для дочернего, а вот методы для
доступа нет
...
abstract class Animal {
    ...
    public Animal(String name) { //хоть нельзя создать экземпляр абстрактного класса, его конструктор будет вызван при создании дочерних объектов
        //  super();//его можно не писать компилятор сам подставит
...
class Dog extends Animal implements Friendly {
    ...
    public Dog(String name, int age) {
        super(name); //если не написать конструктор с парамтером, компилятор подставит вызов род конструктора 
        // по умолчанию (при условии есть в ) Animal есть такой конструктор
    ...
    public void beFriendly() {
        System.out.println("I am friendly?= " + isFriendly);
    }
}
```

Если возникает потребность внутри конструктора вызвать еще один конструктор чтобы избежать дублирования кода, то нужно
использовать либо this() либо super(). Одновременно этого сделать нельзя
